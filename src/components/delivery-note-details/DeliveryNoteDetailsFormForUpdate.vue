            ...
            >
               <FormItem class="col-span-1">
                  ...
                  <Select
                     ...
                     @update:model-value="(e) => getMedicinePrice(e)"
                  >
                     <FormControl>
         ...
         >
            <FormItem v-auto-animate>
               ...
               <NumberField
                  ...
                  class="gap-2"
                  :min="1"
                  :model-value="medicinePrice || value"
                  @update:model-value="
                     (v) => {
                        if (v) {
...
<script setup lang="ts">
   ...
   const medicinePrice = ref<number>();
   const getMedicinePrice = async (medicineId: string) => {
      medicinePrice.value = (
         await MedicineService.getMedicine(medicineId)
      ).price;
      console.log(medicinePrice.value);
   };

   onMounted(async () => {
      medicineSelection.value = await getMedicineData();
      console.log(medicineSelection.value);
   });
   ...
   const onSubmit = handleSubmit((values) => {
      const handleValues = { ...values, price: medicinePrice.value };
